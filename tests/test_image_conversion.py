...
from bossdb_to_tiff_converter import get_indices, intern_convert, cloud_convert
...
def test_get_indices_invalid_range():
    ...
    with pytest.raises(ValueError):
        get_indices('X', coord_dims, '5:15')
    with pytest.raises(ValueError):
        get_indices('Y', coord_dims, '10:5')

# Test intern_convert
@patch('bossdb_to_tiff_converter.array')
def test_intern_convert(mock_array, intern_dataset_mock, tmpdir):
    ...
    intern_convert(f"bossdb://{URL}", URL.replace("/", "_"), RESOLUTION, X_DIMENSIONS, Y_DIMENSIONS, Z_DIMENSIONS, file_path)
    assert len(os.listdir(file_path)) == 100

# Test cloud_convert
@patch('bossdb_to_tiff_converter.CloudVolume')
def test_cloud_convert(mock_cloudvolume, cloud_dataset_mock, tmpdir):
    mock_cloudvolume.return_value = cloud_dataset_mock
    file_path = str(tmpdir.mkdir("test_output"))
    cloud_convert(f"s3://bossdb-open-data/{URL}", URL.replace("/", "_"), RESOLUTION, X_DIMENSIONS, Y_DIMENSIONS, Z_DIMENSIONS, file_path)
    assert len(os.listdir(file_path)) == 100